...
export class DatepickerPage extends HelperBase{
  ...
  async selectDatepickerWithRangeFromToday(
    ...
    await calendarInputField.click();
    const dateToAssertStart = await this.selectDateInTheCalendar2(
      startDayFromToday
    );
    const dateToAssertEnd = await this.selectDateInTheCalendar2(
      endDayFromToday
    );
  ...
  private async selectDateInTheCalendar2(numberOfDaysFromToday: number) {
    let date = new Date();
    date.setDate(date.getDate() + numberOfDaysFromToday);
    const expectedDate = date.getDate().toString();
    const expectedMonthShot = date.toLocaleString("En-US", { month: "short" });
